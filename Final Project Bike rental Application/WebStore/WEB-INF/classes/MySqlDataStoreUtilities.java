...
public class MySqlDataStoreUtilities {
    ...
    public static boolean deleteOrder(int orderId,String orderName) {
        try {
    ...
    public static void insertOrder(int orderId, String userName, String orderName, double orderPrice, String userAddress, String creditCardNo, String Location,double days) {
        try {
            ...
            Double totalcost = days*orderPrice;
            getConnection();

            String insertIntoCustomerOrderQuery = "insert into orders (orderID, userName, orderName, orderPrice, userAddress, creditCardNo, orderTime, Location, totalcost,days) VALUES (?,?,?,?,?,?,?,?,?,?);";
    ...
    public static HashMap<Integer, ArrayList<OrderPayment>> selectOrder() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                OrderPayment order = new OrderPayment(rs.getInt("OrderId"), rs.getString("userName"), rs.getString("orderName"), rs.getDouble("orderPrice"), rs.getString("userAddress"), rs.getString("creditCardNo"),rs.getString("Location"),rs.getDouble("totalcost"),rs.getDouble("days"));
...
public static HashMap<String,Phone> getphone()
{
	...
	try
	{
		...
		while(rs.next())
		{	Phone phone = new Phone(rs.getString("productName"),rs.getDouble("productPrice"),rs.getString("productImage"),rs.getString("productCondition"),rs.getDouble("productDiscount"),rs.getDouble("HrPrice"),rs.getDouble("FullPrice"),rs.getDouble("inventory"));
...
public static HashMap<String, Product> selectOnSale() {
        ...
        try {
            ...
            String selectAcc = "select distinct * from productdetails where productCondition = 'new'";
            PreparedStatement pst = conn.prepareStatement(selectAcc);
          //  pst.setString(1, "1");
            ResultSet rs = pst.executeQuery();
    ...
    public static HashMap<String, Product> selectRebate() {
        ...
        try {
            ...
            String selectAcc = "select distinct * from Productdetails where productDiscount > 0";
            PreparedStatement pst = conn.prepareStatement(selectAcc);
            ResultSet rs = pst.executeQuery();
    ...
    public static HashMap<String, OrderPayment> selectSaleAmount() {
        ...
        try {
            ...
            String selectAcc = "SELECT orderName,count(days) as days,count(orderTime) as soldAmount, orderTime,ROUND(sum(totalcost),2) as totsales,orderPrice from orders group by orderName";
            PreparedStatement pst = conn.prepareStatement(selectAcc);
            ResultSet rs = pst.executeQuery();

            int i = 0;
            while (rs.next()) {
                OrderPayment orderPayment = new OrderPayment(rs.getString("orderName"), rs.getDouble("days"), rs.getInt("soldAmount"),rs.getDate("orderTime"),rs.getDouble("totsales"), rs.getDouble("orderPrice"));
                i++;
    ...
    public static HashMap<String, Product> selectInventory() {
      ...
      try {
          ...
          String selectAcc = "select distinct * from productdetails";
          PreparedStatement pst = conn.prepareStatement(selectAcc);
          ResultSet rs = pst.executeQuery();
  ...
  public static HashMap<String, OrderPayment> selectDailyTransaction() {
          ...
          try {
              ...
              String selectAcc = "SELECT count(orderTime) as soldAmount, orderTime,ROUND(sum(totalcost),2) as totsales from orders group by orderTime";
              PreparedStatement pst = conn.prepareStatement(selectAcc);
              ResultSet rs = pst.executeQuery();

              int i = 0;
              while (rs.next()) {
                  OrderPayment orderPayment = new OrderPayment(rs.getInt("soldAmount"), rs.getDate("orderTime"), rs.getDouble("totsales"));
                  i++;
                  hm.put(String.valueOf(i), orderPayment);
      ...
      public static HashMap<String, OrderPayment> totsalesByInventory() {
          HashMap<String, OrderPayment> hm = new HashMap<String, OrderPayment>();
          try {
              getConnection();

              String selectAcc = "select (temp.orderName),temp.saleAmount,orders.orderPrice from orders, (select orderName, count(orderName) as saleAmount from orders group by orderName) as temp where orders.orderName = temp.orderName";
              PreparedStatement pst = conn.prepareStatement(selectAcc);
              ResultSet rs = pst.executeQuery();
      ...
      public static Double SelecttotalcostofOrder(int orderId) {
          Double totalcost=0.0;
              try {
                  getConnection();

                  String selectAcc = "SELECT ROUND(sum(totalcost),2) as totalcost from orders where OrderId=?"+orderId+" ";
                  PreparedStatement pst = conn.prepareStatement(selectAcc);
                  pst.setInt(1,orderId);
              	  ResultSet rs = pst.executeQuery();
...
return totalcost;
          ...
          public static String updateInventory(String productName)
          {
              String msg = "Inventory is updated successfully";
          	try{

          		getConnection();
          		String updateProductQurey = "UPDATE productdetails set inventory=(inventory)-1 where productName=?;" ;
          			PreparedStatement pst = conn.prepareStatement(updateProductQurey);
          			pst.setString(1,productName);
          			pst.executeUpdate();
          	}
          	...
          	{
          		msg = "Inventory cannot be updated";
          		e.printStackTrace();
...
public static String SelecttotalInventory(String prodname) {
    String pname="";
        try {
            getConnection();

            String selectAcc = "select sum(inventory) as inventory FROM productdetails where productName=?"+" ";
            PreparedStatement pst = conn.prepareStatement(selectAcc);
            pst.setString(1,prodname);
            ResultSet rs = pst.executeQuery();

            while (rs.next()) {
     pname = (rs.getString("inventory"));
              }
              return pname;
        } catch (Exception e) {
        }
return pname;
    ...
    public static String updateInventoryAfterCancel_order(String productName)
    {
        String msg = "Inventory is updated successfully";
      try{
    ...
    public static HashMap<String,Console> getConsoles()
       ...
       ////////////////////////////
       public static String getProductPrice(String prodname) {
           String pname="";
               try {
                   getConnection();

                   String selectAcc = "SELECT productprice from productdetails where productName=? "+" LIMIT 1";
                   PreparedStatement pst = conn.prepareStatement(selectAcc);
                   pst.setString(1,prodname);
                   ResultSet rs = pst.executeQuery();

                   while (rs.next()) {
            pname = (rs.getString("productprice"));
                     }
                     return pname;
               } catch (Exception e) {
               }
