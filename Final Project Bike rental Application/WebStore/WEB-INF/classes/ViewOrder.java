			...
			if (request.getParameter("orderId") != null && request.getParameter("orderId") != "" )
			{
				...
				if(size>0)
				{
					...
					for (OrderPayment oi : orderPayments.get(orderId))
					{
						...
						pw.print("<td>"+oi.getOrderId()+".</td><td>"+oi.getUserName()+"</td><td>"+oi.getOrderName()+"</td><td>Price: "+oi.getTotalCost()+"</td><td>"+oi.getDays()+"</td>");
				...
				for (OrderPayment oi : orderPayments.get(orderId))
					{
							if(oi.getOrderName().equals(orderName) && oi.getUserName().equals(username))
							{
								...
								utility.updateInventoryAfterCancel_order(productName);
							}
					}
